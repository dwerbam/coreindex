...
class UtxoDB:
    ...
    def update(self, new_utxos_df: pl.DataFrame, spent_utxos_df: pl.DataFrame) -> set:
        ...
        # Add partition key to inputs
        if not new_utxos_df.is_empty():
            new_utxos_df = new_utxos_df.with_columns(
                pl.col("tx_hash").str.slice(0, 2).alias("pkey")
            )
    ...
    def get_utxos_for_tx_hashes(self, tx_hashes: list) -> pl.DataFrame:
        """
        Retrieves UTXOs for a list of transaction hashes.
        ...
        for pkey in pkeys:
            ...
            # This is faster than loading everything if the partition is large
            # But here we load the partition.
            # Ideally we use scan_parquet but for batch processing read might be fine if partitions are small (48MB).
            # Let's use scan/collect for potentially better filtering pushdown.
